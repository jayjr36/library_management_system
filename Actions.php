...
Class Actions extends DBConnection{
    ...
    function faculty_student_login(){
        ...
        if (!$result) {
            ...
            if (!$row) {
                ...
                $resp['status'] = "failed";
                $resp['msg'] = "Your account has been blocked by the management. Please contact support for assistance.";
            } else {
                $resp['status'] = "success";
    ...
    function save_category(){
        ...
        @$checkResult = $this->query("SELECT COUNT(category_id) as count from `category_list` where `name` = '{$name}' ".($id > 0 ? " and category_id != '{$id}'" : ""));
        $checkRow = $checkResult->fetch_assoc();
        $check = isset($checkRow['count']) ? $checkRow['count'] : 0;
    ...
    function save_sub_category(){
        ...
        @$checkResult = $this->query("SELECT COUNT(sub_category_id) as count from `sub_category_list` where `name` = '{$name}' and `category_id` = '{$category_id}' ".($id > 0 ? " and sub_category_id != '{$id}'" : ""));
        $checkRow = $checkResult->fetch_assoc();
        $check = isset($checkRow['count']) ? $checkRow['count'] : 0;
    ...
    function save_admin(){
        ...
        @$checkResult = $this->query("SELECT count(admin_id) as `count` FROM admin_list where `username` = '{$username}' ".($id > 0 ? " and admin_id != '{$id}' " : ""));
        $checkRow = $checkResult->fetch_assoc();
        $check = isset($checkRow['count']) ? $checkRow['count'] : 0;
        ...
        if(@$check> 0){
            ...
            if($save){
                ...
                $resp['status'] = 'failed';
                $resp['msg'] = 'Saving Admin User Details Failed. Error: '. $this->$conn->error;
                $resp['sql'] =$sql;
            }
        }
        return json_encode($resp);
    ...
    function save_user(){
        ...
        @$checkResult = $this->query("SELECT count(user_id) as `count` FROM user_list where `username` = '{$username}' ".($id > 0 ? " and user_id != '{$id}' " : ""));
$checkRow = $checkResult->fetch_assoc();
$check = isset($checkRow['count']) ? $checkRow['count'] : 0;
        ...
        if(@$check> 0){
            ...
            if($save){
                ...
                $resp['status'] = 'failed';
                $resp['msg'] = 'Saving Details Failed. Error: '. $this->$conn->error;
                $resp['sql'] =$sql;
            }
        }
        return json_encode($resp);
    ...
    function delete_user(){
        ...
        if($delete){
            ...
            $resp['status']='failed';
            $resp['error']=$this-> $this->$conn->error;
        }
        return json_encode($resp);
    }
    function save_book(){
        extract($_POST);
        $result = $this->query("SELECT count(book_id) as `count` FROM `book_list` where `isbn` = '{$name}' ".($id > 0 ? " and book_id != '{$id}'" : ''));
        $row = $result->fetch_assoc();
        $check = $row['count'];
         if($check> 0){
            $resp['status'] = 'failed';
            ...
            if($save){
                ...
                if(empty($id))
               $result = $this->query("SELECT max(book_id) as last_id from `book_list`");
$row = $result->fetch_assoc();
$last_id = $row['last_id'];
    ...
    function delete_book(){
        ...
        if($delete){
            ...
            $resp['status']='failed';
            $resp['msg'] = 'An error occure. Error: '. $this->$conn->error;
        }
        return json_encode($resp);
    }
    function delete_img(){
    ...
    function add_to_cart(){
        ...
        $result = $this->query("SELECT count(book_id) as `count` FROM `cart_list` where `book_id` = '{$book_id}' and `user_id` = '{$user_id}'");
        $row = $result->fetch_assoc();
        $check = $row['count'];
        ...
        if($check > 0){
            ...
            if($save){
                $resp['status'] ='success';
                $result = $this->query("SELECT count(book_id) as total FROM `cart_list` where `user_id` = '{$user_id}'");
                $row = $result->fetch_assoc();
                $count = $row['total'];
    ...
    function update_cart(){
        ...
        if($save){
            $resp['status'] ='success';
            $result = $this->query("SELECT SUM(quantity) as total FROM `cart_list` where `user_id` = '{$user_id}'");
            $row = $result->fetch_assoc();
            $count = $row['total'];
...
$count = $row['total'];
    ...
    function book_to_borrow(){
        ...
        while(true){
            ...
            $result = $this->query("SELECT count(borrowed_id) as `count` from `borrowed_list` where transaction_code = '{$code}' ");
            $row = $result->fetch_assoc();
            $check = $row['count'];
    ...
    function update_borrowed_status(){
        ...
        if($update){
            ...
            $resp['status']='failed';
            $resp['msg'] = "Failed to Update status. Error: ". $this->$conn->error;
            $resp['sql'] = $sql;
        }
        return json_encode($resp);
    }
